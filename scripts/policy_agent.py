...
def REINFORCE(training_pairs, policy_nn, num_episodes):
    ...
    for i_episode in range(num_episodes):
        ...
        if len(state_batch_negative) != 0:
            ...
            policy_nn.update(
                np.reshape(state_batch_negative, (-1, state_dim)),
                -0.05,
                action_batch_negative,
            )
        ...
        if done == 1:
            ...
            policy_nn.update(
                np.reshape(state_batch, (-1, state_dim)), total_reward, action_batch
            )
        else:
            global_reward = -0.05
            ...
            try:
                ...
                for item in good_episodes:
                    ...
                    policy_nn.update(
                        np.squeeze(teacher_state_batch), 1, teacher_action_batch
                    )
...
def test():
    ...
    with tf.Session() as sess:
        ...
        for episode in range(test_num):
            ...
            if done:
                if len(path_set) != 0:
                    path_found_embedding = [
                        env.path_embedding(path.split(" -> ")) for path in path_set
                    ]
                    curr_path_embedding = env.path_embedding(env.path_relations)
                    ...
                    policy_network.update(
                        np.reshape(state_batch, (-1, state_dim)),
                        0.1 * diverse_reward,
                        action_batch,
                    )
